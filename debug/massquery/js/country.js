$(document).ready(function(){
    ...
    //r360.config.serviceUrl   = 'http://api-us.route360.net/na_southwest/';
 ...
 */
    ...
    var to = r360.travelOptions();

    to.addSource(markers[0]);
    // add all the museums to the options
    to.setTargets(markers);
    // lets show polygons for 5 to 30 minutes
    to.setTravelTimes([300, 600, 900, 1200, 1500, 1800]);
    // set the travel type to transit
    to.setTravelType('walk');
    // for all museums which are not reachable within <maxRoutingTime>
    // no routing time will be returned
    to.setMaxRoutingTime(3600);

    r360.TimeService.getRouteTime(to, function(sources){
        console.log(sources);
    });
