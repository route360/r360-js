$(document).ready(function(){
    ...
    var tileLayer = L.tileLayer('http://{s}.tile.osm.org/{z}/{x}/{y}.png', {
        maxZoom: 22, attribution: "" }).addTo(map);

    var PLACES_API_URL = "http://dev.route360.net/places/places";
    ...
    map.on('click', function(e){
        ...
        if (  index0 >= 0 || index1 >= 0 || index2 >= 0 ) {
            ...
            // createMarker([52.520033571173414, 13.387184143066406], 'home',  min == 0 ? 'red' : (min == 1 ? 'orange' : 'blue'), sourceLayer, updateSource, 'undefined', min);
            // createMarker([52.49955597339877, 13.374567031860352], 'home',  min == 0 ? 'red' : (min == 1 ? 'orange' : 'blue'), sourceLayer, updateSource, 'undefined', min);
            createMarker([52.521456722976616, 13.411431312561035], 'home',  min == 0 ? 'red' : (min == 1 ? 'orange' : 'blue'), sourceLayer, updateSource, 'undefined', min);
            createMarker([52.50681825421263, 13.332166671752928], 'home',  min == 0 ? 'red' : (min == 1 ? 'orange' : 'blue'), sourceLayer, updateSource, 'undefined', min);
            createMarker([52.54864442653698, 13.388428688049316], 'home',  min == 0 ? 'red' : (min == 1 ? 'orange' : 'blue'), sourceLayer, updateSource, 'undefined', min);
    ...
    function createMarker(latLng, icon, color, layer, dragend, popup, index){
        ...
        marker.on('dragend', function(e) { console.log(e.target)});
    ...
    function getPolygons(){
        ...
        if (  index0 == -1 || index1 == -1 || index2 == -1 ) {
            ...
            //         var sourceMarker        = sourceMarkers[i];
            //         sourceMarker.travelType = travelType;
            //         travelOptions.addSource(sourceMarker);            
            //     }
            // }
            // 
            
        // 52.520033571173414, 13.387184143066406
        // 52.521456722976616, 13.411431312561035
        // 52.49955597339877, 13.374567031860352

            travelOptions.addSource({ lat : 52.521456722976616, lng : 13.411431312561035, travelType : travelType });   
            travelOptions.addSource({ lat : 52.50681825421263, lng : 13.332166671752928, travelType : travelType });   
            travelOptions.addSource({ lat : 52.54864442653698, lng : 13.388428688049316, travelType : travelType });   


// [52.521456722976616, 13.411431312561035]
// [52.50681825421263, 13.332166671752928],
// [52.54864442653698, 13.388428688049316],
            ...
            if ( travelOptions.getSources().length > 0 ) {
                ...
                // travelOptions.setTravelTimes([120, 240, 360, 480, 600, 720]);
                travelOptions.setWaitControl(waitControl);
                travelOptions.setElevationEnabled(true);
                ...
                var maxTravelTime1 = maxTravelTime;

                if ( maxTravelTime1 == 1200 || maxTravelTime1 == 2400 )
                    travelOptions.setMinPolygonHoleSize(10 * 1000 * 1000);
                if ( maxTravelTime1 == 3600 || maxTravelTime1 == 4800 )
                    travelOptions.setMinPolygonHoleSize(100 * 1000 * 1000);
                if ( maxTravelTime1 == 6000 || maxTravelTime1 == 7200 )
                    travelOptions.setMinPolygonHoleSize(1000 * 1000 * 1000);

                // if ( r360.config.defaultPolygonLayerOptions.inverse ) 
                    // travelOptions.setTravelTimes([maxTravelTime1]);
    ...
    function getPlaces() {
            ...
            waitControl.updateText("Frage Objekte an ...")
            waitControl.show();
            ...
            var parameters = [
                "lat="+52.51601193890388,
                "lng="+13.390617370605467,
                "radius="+10000,
                "northEast="+NE.lat+'|'+NE.lng,
                "southWest="+SW.lat+'|'+SW.lng,
                "timeout="+10000,
                "types=osm:tourism='museum'"
            ];

            $.get(PLACES_API_URL+ "?" + parameters.join("&")).then(function(response){
                ...
                // no need to make time query 
                // if ( places.length == 0 ) { requestPending = false; return; }

                r360.config.i18n.timeWait.en = "Berechne Reisezeit zu " + places.length + " Zielen ...";
                ...
                travelOptions.setTravelTimes([maxTravelTime]);
                travelOptions.setWaitControl(waitControl);
                travelOptions.setDate(date);
                travelOptions.setTime(time);
                travelOptions.setMaxRoutingTime(maxTravelTime);
                ...
                r360.TimeService.getRouteTime(travelOptions, function(sources){
                    ...
                    var targets = _.sortBy(sources[0].targets, 'travelTime');
                    ...
                    _.each(targets, function(target){
                        ...
                        if ( place.travelTime > 0 && place.travelTime <= travelOptions.getMaxRoutingTime() ) {
                            ...
                            place.marker = L.marker([place.lat, place.lng], { icon: L.AwesomeMarkers.icon({ icon: 'fa fa-star', prefix : 'fa', markerColor: 'green' }), opacity : 1 })
